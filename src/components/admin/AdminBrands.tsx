...
export const AdminBrands: React.FC = () => {
  ...
  const loadBrands = async () => {
    ...
    if (data.length === 0) {
       const initial: Omit<Brand, 'id' | 'createdAt' | 'updatedAt'> = {
         name: 'VANTA',
         slug: 'vanta',
         description: 'The flagship internal brand.',
         logo: '',
         status: 'active',
         displayOrder: 1
       };
       await brandService.create(initial);
       const newData = await brandService.getAll();
       setBrands(newData);
    } else {
       setBrands(data);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingBrand) {
      const success = await brandService.update(editingBrand.id, formData);
      if (success) {
        addToast('Brand Updated', `${formData.name} updated successfully.`, 'success');
      }
    } else {
      const created = await brandService.create(formData);
      if (created) {
        addToast('Brand Created', `${formData.name} created successfully.`, 'success');
      }
    }
    setIsFormOpen(false);
    loadBrands();
  ...
  const handleDelete = async (id: string, name: string) => {
    if (window.confirm(`Are you sure you want to delete ${name}?`)) {
      ...
      if (success) {
        addToast('Brand Deleted', `${name} has been removed.`, 'info');
        loadBrands();
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">Brands</h1>
          <p className="text-sm text-zinc-400 mt-1">Manage brand partners and internal labels.</p>
        </div>
        <button 
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-zinc-800/50">
              {isLoading ? (
                ...
                <tr key={brand.id} className="hover:bg-zinc-900/30 transition-colors group">
                  <td className="px-6 py-4 font-mono text-zinc-500">{brand.displayOrder}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-zinc-800 border border-zinc-700 overflow-hidden shrink-0">
                        {brand.logo ? <img src={brand.logo} alt={brand.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center font-bold text-zinc-600">{brand.name.charAt(0)}</div>}
                      </div>
                      <div>
                        <p className="font-bold text-white">{brand.name}</p>
                        <p className="text-[10px] text-zinc-500 mt-0.5">/{brand.slug}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-[10px] uppercase font-bold tracking-wider ${brand.status === 'active' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-zinc-800 text-zinc-400'}`}>
                      {brand.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleOpenForm(brand)} className="p-2 rounded-lg hover:bg-zinc-800 text-zinc-400 transition-colors">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(brand.id, brand.name)} className="p-2 rounded-lg hover:bg-rose-500/10 text-rose-400 transition-colors">
              ...
              {!isLoading && filteredBrands.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-zinc-500">No brands found.</td>
                </tr>
              )}
            </tbody>
          </table>
