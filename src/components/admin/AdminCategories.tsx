...
export const AdminCategories: React.FC = () => {
  ...
  const loadCategories = async () => {
    ...
    // Use some default if empty to help protoype
    if (data.length === 0) {
       const initial: Omit<Category, 'id' | 'createdAt' | 'updatedAt'> = {
         name: 'Running',
         slug: 'running',
       ...
       await categoryService.create(initial);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingCategory) {
      const success = await categoryService.update(editingCategory.id, formData);
      if (success) {
        addToast('Category Updated', `${formData.name} updated successfully.`, 'success');
      }
    } else {
      const created = await categoryService.create(formData);
      if (created) {
        addToast('Category Created', `${formData.name} created successfully.`, 'success');
      }
    }
    setIsFormOpen(false);
    loadCategories();
  ...
  const handleDelete = async (id: string, name: string) => {
    if (window.confirm(`Are you sure you want to delete ${name}?`)) {
      ...
      if (success) {
        addToast('Category Deleted', `${name} has been removed.`, 'info');
        loadCategories();
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">Categories</h1>
          <p className="text-sm text-zinc-400 mt-1">Organize products into high-level groups.</p>
        </div>
        <button 
            ...
            <tbody className="divide-y divide-zinc-800/50">
              {isLoading ? (
                ...
                <tr key={cat.id} className="hover:bg-zinc-900/30 transition-colors group">
                  <td className="px-6 py-4 font-mono text-zinc-500">{cat.displayOrder}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-zinc-800 border border-zinc-700 overflow-hidden shrink-0">
                        {cat.image ? <img src={cat.image} alt={cat.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center font-bold text-zinc-600">{cat.name.charAt(0)}</div>}
                      </div>
                      <div>
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-[10px] uppercase font-bold tracking-wider ${cat.status === 'active' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-zinc-800 text-zinc-400'}`}>
                      {cat.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleOpenForm(cat)} className="p-2 rounded-lg hover:bg-zinc-800 text-zinc-400 transition-colors">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(cat.id, cat.name)} className="p-2 rounded-lg hover:bg-rose-500/10 text-rose-400 transition-colors">
              ...
              {!isLoading && filteredCategories.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-zinc-500">No categories found.</td>
                </tr>
              )}
            </tbody>
          </table>
